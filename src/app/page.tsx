...
export default function Home() {
  return (
    <main className="min-h-screen overflow-hidden bg-[#090b10] text-[#f5efe2]">
          ...
          <div className="grid flex-1 items-center gap-10 py-10 lg:grid-cols-[0.92fr_1.08fr]">
            <div>
              ...
              <p className="mt-6 max-w-2xl text-base leading-8 text-[#d8cdb9] sm:text-lg">
                输入出生信息、MBTI 和你最近最困惑的问题。系统会从八字节律、紫微结构、星座情绪与行为模式四个角度，先免费给你一份能看懂、能验证的核心诊断。
              </p>
            ...
            <div className="grid gap-5">
              ...
              <article className="xj-report-preview p-5">
                ...
                <div className="mt-5 grid gap-2 border-t border-[#d7aa55]/18 pt-4">
                  {["先指出反复卡住你的底层模式", "再解释这种模式在事业与关系中的表现", "最后给出一个今天就能开始的行动"].map((item) => (
                    <p key={item} className="text-sm font-semibold text-[#f2ddae]">⊙ {item}</p>
                  ))}
                </div>
                <a
                  href="#report-form"
                  className="mt-5 flex h-12 items-center justify-center rounded-full border border-[#d7aa55]/35 bg-[#d7aa55]/10 text-sm font-bold text-[#f2ddae] transition hover:border-[#d7aa55] hover:bg-[#d7aa55] hover:text-[#121714]"
                >
                  生成我的免费核心诊断
                </a>
                <p className="mt-3 text-center text-xs text-[#cfc2ae]">先看结果是否对你有帮助，再决定是否继续深入</p>
              </article>
            </div>
          </div>
        </div>
      ...
      <section className="border-b border-[#d7aa55]/18 bg-[#07080d]">
        <div className="mx-auto max-w-5xl px-5 py-14 sm:px-8 lg:px-10">
          <div className="mb-8 text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-[#cfc2ae]">
              整个过程约 2 分钟。免费报告会给出核心人格模式、四维交叉判断和一个可立即执行的建议，不会在填写过程中要求付款。
            </p>
          </div>
          <MysticReportForm />
        </div>
      ...
      <section id="how-it-works" className="border-b border-[#d7aa55]/18 bg-[#0b0b12]">
